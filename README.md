...
- `lib/providers/geocoding.ts` + `lib/providers/transport/ground-transport.ts` — car/bus/train options for **any** origin/destination, not just `lib/cityData.ts`'s ~130 airport cities: geocodes free text via OSM Nominatim (falls back from the curated city list, no key needed), gets a real driving distance/duration from OpenRouteService (`OPENROUTESERVICE_API_KEY`), and derives clearly-labeled estimated prices from that real distance — no self-serve API returns real worldwide point-to-point train/bus/shuttle fares
