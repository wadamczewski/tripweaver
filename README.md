...
- `lib/optimizer/agent-review.ts` — the Trip Optimizer agent (OpenRouter-backed, heuristic fallback), reviewed via `components/optimizer/OptimizerAgentReview.tsx`, shown as a collapsible card in the results grid's third (right) column that auto-expands once a review completes — showing the actual recommended trip (image, price, provider links) plus separate "Why this trip" and "Trade-offs" subsections (the agent returns these as distinct fields, not one blended paragraph), blurred with a spinner overlay while a new review is in flight. Its ranking is authoritative: `applyAgentRanking()` in `lib/scoring.ts` reorders the displayed trip list to match. Its data-gap/caveat notes (`review.warnings`) render just below it in the same right column (`AgentNotesPanel` in `app/page.tsx`)
- `components/optimizer/OptimizerPanel.tsx` — the left-column priority controls: five Low/Balanced/High segmented toggles per factor (not free-dragging percent sliders — see `PROJECT_STATUS.md`'s 2026-08-03 note for why) driving a live weighted score and a real per-factor score breakdown for the recommended trip, plus the "Review updated ranking" button (centered under "Reset to defaults") that triggers a fresh agent review — kept next to the controls that actually affect it rather than inside the agent card itself. No fabricated "what if" comparison cards — everything shown is computed from the actual result set. The results page uses a real three-column CSS grid (`400px_minmax(0,1fr)_320px`, both side columns sticky) from `lg:` (1024px) up, widened to `max-w-[1800px]` so the third column never shrinks the center results column
