...
- `lib/providers/` — real adapters (`flights/`, `accommodations/`, `packages/`); `transport/` and `accommodation/` also hold unused reference mock providers from the original hackathon build, not wired into `lib/providers/index.ts`
- `lib/optimizer/agent-review.ts` — the Trip Optimizer agent (OpenRouter-backed, heuristic fallback), reviewed via `components/optimizer/OptimizerAgentReview.tsx`, shown as a one-line collapsible bar at the top of the results column that auto-expands once a review completes — showing the actual recommended trip (image, price, provider links) plus separate "Why this trip" and "Trade-offs" subsections (the agent returns these as distinct fields, not one blended paragraph), blurred with a spinner overlay while a new review is in flight. Its ranking is authoritative: `applyAgentRanking()` in `lib/scoring.ts` reorders the displayed trip list to match
...
- "Preferred transport" only has real effect for Flight — Duffel is the only real transport provider and it's flights-only, so selecting Train/Bus/Car/Ferry alone correctly returns zero real results rather than substituting flights, but can't produce real non-flight data that doesn't exist.
